...
@push('breadcrumbs')
    ...
    <!--end::Item-->
    <!--begin::Item-->
    <li class="breadcrumb-item active"><a href="" class="text-muted text-hover-primary">Edit Kit Library</a></li>
    <!--end::Item-->
@endpush
@section('content')
    <!--begin::Content-->
                    ...
                    <div class="card">
                        ...
                        <div class="card-header border-0 pt-6">
                            ...
                            <div class="card-title">
                                <h2>Edit Kit Library</h2>
                            </div>
                        </div>
                        <!--end::Card header-->
                        <!--begin::Card body-->
                        <div class="card-body pt-0">
                            <form class="form" action="#" id="editKitLibraryForm" enctype="multipart/form-data" method="POST">
                                <input type="hidden" name="id" value="{{ $kit_library->id }}">
                                <!--begin::Modal body-->
                                <div class="modal-body py-10 px-lg-17">
                                    <div class="row g-9 mb-7">
                                        <div class="col-md-8 fv-row fv-plugins-icon-container">
                                            <label class="required fs-6 fw-semibold mb-2">Title</label>
                                            <input type="text" class="form-control" placeholder="Title" name="title" value="{{ $kit_library->title }}">
                                            <div class="fv-plugins-message-container invalid-feedback"></div>
                                        </div>
                                        <div class="col-md-4 fv-row fv-plugins-icon-container">
                                            <label class="required fs-6 fw-semibold mb-2">Preview Link</label>
                                            <input type="url" class="form-control" placeholder="Preview Link" name="preview_link" value="{{ $kit_library->preview_link }}">
                                            <div class="fv-plugins-message-container invalid-feedback"></div>
                                        </div>
                                        <div class="col-md-6 fv-row fv-plugins-icon-container">
                                            ...
                                            <select class="form-select" id="kit_type" name="kit_type" data-control="select2" data-placeholder="Select Any" data-allow-clear="true">
                                                ...
                                                @foreach($kit_types as $kit_type)
                                                    <option value="{{ $kit_type->id }}" @if($kit_library->kit_type_id == $kit_type->id) selected @endif>{{ $kit_type->title }}</option>
                                                @endforeach
                                            </select>
                                            <div class="fv-plugins-message-container invalid-feedback"></div>
                                        </div>
                                        <div class="col-md-6 fv-row fv-plugins-icon-container">
                                            ...
                                            <select class="form-select" id="category" name="category" data-control="select2" data-placeholder="Select Any" data-allow-clear="true">
                                                ...
                                                @foreach($categories as $category)
                                                    <option value="{{ $category->id }}" @if($kit_library->category_id == $category->id) selected @endif>{{ $category->title }}</option>
                                                @endforeach
                                            </select>
                                            <div class="fv-plugins-message-container invalid-feedback"></div>
                                        </div>
                                        <div class="col-md-12 fv-row fv-plugins-icon-container">
                                            ...
                                            <select class="form-select" id="tags" name="tags[]" data-control="select2" data-placeholder="Select Any" data-allow-clear="true"  multiple="multiple">
                                                <option></option>
                                                @foreach($tags as $tag)
                                                    <option value="{{ $tag->id }}" {{ (in_array($tag->id, $assigned_tags_ids) ) ? 'selected ' : '' }}>{{ $tag->title }}</option>
                                                @endforeach
                                            </select>
                                            <div class="fv-plugins-message-container invalid-feedback"></div>
                                        </div>
                                        <div class="col-md-12 fv-row fv-plugins-icon-container">
                                            ...
                                            <select class="form-select" id="packages" name="packages[]" data-control="select2" data-placeholder="Select Any" data-allow-clear="true"  multiple="multiple">
                                                ...
                                                @foreach($packages as $package)
                                                    <option value="{{ $package->id }}" {{ (in_array($package->id, $assigned_packages_ids) ) ? 'selected ' : '' }}>{{ $package->title }}</option>
                                                @endforeach
                                            </select>
                                            <div class="fv-plugins-message-container invalid-feedback"></div>
                                        </div>
                                        <div class="col-md-12 fv-row fv-plugins-icon-container">
                                            ...
                                            <textarea  id="description" name="description" placeholder="Description">{{ $kit_library->description }}</textarea> <!--end::Input-->
                                            <div class="fv-plugins-message-container invalid-feedback"></div>
                                        </div>
                                        <div class="col-md-6 fv-row fv-plugins-icon-container">
                                            <label class="required fs-6 fw-semibold mb-2">Image (1200px X 800px)</label> <br>
                                            <div class="image-input image-input-empty" data-kt-image-input="true" style="background-image: url('{{ ($kit_library->image) ? asset($kit_library->getThumbImage()) : asset('assets/media/svg/files/blank-image.svg') }}')">
                                        ...
                                        <div class="col-md-6 fv-row fv-plugins-icon-container">
                                            ...
                                            <div class="image-input image-input-empty" data-kt-image-input="true" style="background-image: url('{{ ($kit_library->image) ? asset($kit_library->getCoverThumbImage()) : asset('assets/media/svg/files/blank-image.svg') }}')">
                                                <div class="image-input-wrapper w-125px h-125px"></div>
                                                <label class="btn btn-icon btn-circle btn-color-muted btn-active-color-primary w-25px h-25px bg-body shadow"
                                                       data-kt-image-input-action="change"
                                                       data-bs-toggle="tooltip"
                                    ...
                                    <div class="row g-9 mb-7">
                                        <h4>Features</h4>
                                              @foreach($kit_library->kit_library_features as $key => $kit_library_feature)
                                                    <input type="hidden" name="kit_library_feature_id[]" value="{{ $kit_library_feature->id }}">
                                                <div class="form-group row m-5">
                                                    <div class="col-md-3">
                                                        <label class="form-label">Title</label>
                                                        <input type="text"  class="form-control" placeholder="Title" name="kit_library_feature_title[]" value="{{ $kit_library_feature->title }}" />
                                                    </div>
                                                    <div class="col-md-4">
                                                        <label class="form-label">{{ __('messages.kit_library_feature') }} ({{ __('messages.en') }})</label>
                                                       <textarea class="form-control" placeholder="Description"  name="kit_library_feature_description[]">{{ $kit_library_feature->description }}</textarea>
                                                    </div>
                                                    <div class="col-md-4">
                                                        <label class="form-label">Icon (SVG Code)</label>
                                                        <textarea class="form-control" placeholder="Icon (SVG Code)" name="kit_library_feature_icon[]">{{ $kit_library_feature->icon }}</textarea>                                                    </div>
                                                    <div class="col-md-1">
                                                        <a href="javascript:;" class="btn btn-sm btn-light-danger mt-3 mt-md-8" onclick="deleteKitLibraryFeature({{ $kit_library_feature->id }})">
...
@push('footer-scripts')
    ...
    <script src="{{ asset('assets/js/admin/kit_libraries/edit_kit_libraries.js') }}"></script>
