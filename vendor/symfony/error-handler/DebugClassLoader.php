...
class DebugClassLoader
{
    ...
    public function checkAnnotations(\ReflectionClass $refl, string $class): array
    {
        ...
        // Don't trigger deprecations for classes in the same user
        if ($class !== $className) {
            ...
            if (isset(self::$method[$use])) {
                if ($refl->isAbstract()) {
                    ...
                    if (!strncmp($vendor, str_replace('_', '\\', $use), $vendorLen)
                        ...
                        // skip "same user" @method deprecations for Symfony\* classes unless symfony/symfony is being tested
                        continue;
                    }
        ...
        foreach ($refl->getMethods() as $method) {
            ...
            if ($canAddReturnType = null !== $forcePatchTypes && !str_contains($method->getFileName(), \DIRECTORY_SEPARATOR.'user'.\DIRECTORY_SEPARATOR)) {
