...
class RouteListCommand extends Command
{
    ...
    protected function isVendorRoute(Route $route)
    {
        ...
        return str_starts_with($path, base_path('user'));
    }
    ...
    protected function filterRoute(array $route)
    {
        if (($this->option('name') && ! Str::contains((string) $route['name'], $this->option('name'))) ||
            ...
            ($this->option('except-user') && $route['user']) ||
            ($this->option('only-user') && ! $route['user'])) {
            return;
        }
    ...
    protected function formatActionForCli($route)
    {
        ...
        if (class_exists($actionClass) && str_starts_with((new ReflectionClass($actionClass))->getFilename(), base_path('user'))) {
    ...
    protected function getOptions()
    {
        return [
            ...
            ['except-user', null, InputOption::VALUE_NONE, 'Do not display routes defined by user packages'],
            ['only-user', null, InputOption::VALUE_NONE, 'Only display routes defined by user packages'],
        ];
    }
}
